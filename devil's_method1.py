...
def find_square_contours2(contours, image_for_area):
    ...
    checkpoint_area = int(full_area / 4.5)
    min_checkpoint_area = int(checkpoint_area / 6)
    square_contours = []
    area = []
    for cnt in contours:
        peri = cv2.arcLength(cnt, True)
...
def find_square_contours3(contours, image_for_area):
    full_area = len(image_for_area) * len(image_for_area[0])
    checkpoint_area = int(full_area / 4.5)
    min_checkpoint_area = int(checkpoint_area / 6)
    square_contours = []
    ...
    for cnt in contours:
		...
		# equal to one, otherwise, the shape is a rectangle
            if ar >= 0.95 and ar <= 1.05:
                # area.append(cv2.contourArea(cnt))#and cv2.contourArea(cnt) > (checkpoint_area / 3):
                area.append(cv2.contourArea(cnt))
                square_contours.append(cnt)
			# shape = "square" if ar >= 0.95 and ar <= 1.05 else "rectangle"
    return square_contours, area
...
while(1):
    ...
    if s == 1:
        c1, _ = find_contours(field2.gray_image().copy(), canny_low, canny_high, gs, gs, 'external')
    ...
    if s == 3: #for define checkpoint contour
        ...
        pre_checkpoint_contour = find_contour_by_area(c2, min_area, max_area) #checkpoint_contour = [contours]
    ...
    if s == 5:
        ...
        img = draw_contours(field2.blank_image_with_same_size().copy(), palette_contour, 1)
        cv2.imshow('Select Only Palette',img)
    if s == 6:
        img2[ : : ] = 1
        cv2.imshow('Select Only Palette',img2)
        palette_contour_for_path_height, _ = find_contours(img, 40, 100, 3, 3, 'external')
...
lo
field2.checkpoint_location
p = find_endpoint(path, field2.checkpoint_location)
triangle.raw_image.shape[1]
len(triangle.raw_image)
### give start (y1,x1) and end (y2,x2) and the binary maze image as input
path2, cost = shortest_path([56, 354], [367, 80], path)
path2
bg = field2.raw_image.copy()
for p in path2:
    bg[p[0]][p[1]] = [0, 0, 255]
len(palette_contour_for_path_height)
# def find_height(palette_contour, all_point, denoised_field_image):
#
#     return path_for_pic
def find_min_max_palette(palette_contour_for_path_height, gray_denoised_field_image):
    ...
    for y in range(len(gray_denoised_field_iamge)):
        for x in range(len(gray_denoised_field_image)):
            dist = cv2.pointPolygonTest(palette_contour_for_path_height, (x, y), False)
            if dist == 1:
